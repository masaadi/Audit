...
class Approved_report extends BaseController
{
    public function __construct()
    {
        parent::__construct();
        if ($this->dx_auth->is_logged_in()): else: redirect(''); endif;
        $this->load->model('reports/approved_report_model', 'personal_model');
    }
    ...
    public function index(){
        ...
        $config['base_url'] = base_url() . 'reports/approved_report/index';
		...
		$this->data['dynamicView'] = "approved_report/index";
   ...
   public function paginate_data()
    {
        ...
        $config['base_url'] = base_url() . 'reports/approved_report/paginate_data';
        $config['total_rows'] = $totalRec;
        $config['per_page'] = $this->total_row;
        $config['link_func'] = 'searchFilter';
        $this->ajax_pagination->initialize($config);
        ...
        $this->load->view('reports/approved_report/home', $data);
    ...
    public function generateListPdf(){
        ...
        $data['wings'] = $this->db->select('service_register.*,service_type.service_type,personal_info.employee_name')
                        ...
                        ->where("service_register.status", "completed")
                        ->order_by("service_register.id", "desc")
                        ->get('service_register')->result_array();
        $data['pdf'] = 1;
        $html = $this->load->view('reports/approved_report/pdf', $data, true);
        $mpdf->SetHtmlHeader('');
        $mpdf->WriteHTML($html);
        $mpdf->Output('approved_services.pdf', 'D');
        exit;
    }
