			...
			if (empty($error)) {
				...
				try {
					...
					foreach ($allDataInSheet as $value) {
						...
						$eid = $value['C'];
						$pid = 0;
						$pArray = $this->Shows->getValue("EMPLOYEE_ID = '$eid' AND STATUS=1", "HR_PERSONALINFO", "ID");
						if (count($pArray)) {
							$pid = $pArray[0]->ID;
						}
						if ($pid > 0) {
							$data = array(
								...
								'FIRST_PF' => $value['F'],
								'SECOND_PF' => $value['G'],
								'INTEREST_STATUS' => $value['H'],
								'CPF_NO' => $value['D'],
								'CREATED_BY' => $this->userId,

							);
							$check = $this->Shows->getValue("EMPLOYEE_ID = '$eid'", "CPF_EMPLOYEE_PF", "ID");
							if (!count($check)) {
								$result = $this->Shows->insertThisValue($data, "CPF_EMPLOYEE_PF");
							}
