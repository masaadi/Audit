...
class Department extends BaseController
{
    public function __construct()
    {
       ...
       // $this->dx_auth->check_uri_permissions();
        ...
        $this->load->model('admin/department_model', 'D_model');
    }
    ...
    public function index()
    {
		...
		$this->data['dynamicView'] = "department/index";
   ...
   public function paginate_data()
    {
        ...
        if (!empty($name)) {
            $conditions['search']['dept_name'] = $name;
        }
        $totalRec = @count($this->D_model->get_user_data($conditions));

        //pagination configuration
        $config['target'] = '#departmentList';
        $config['base_url'] = base_url() . 'admin/department/paginate_data';
        $config['total_rows'] = $totalRec;
        $config['per_page'] = $this->total_row;
        $config['link_func'] = 'searchFilter';
        $this->ajax_pagination->initialize($config);
        ...
        $this->load->view('admin/department/home', $data);
	...
	public function add()
    {
        ...
        $this->load->view('admin/department/entry');
	...
	function added()
    {
        ...
        $this->form_validation->set_rules('department_name', 'Department', 'trim|required|is_unique[department.department_name]|xss_clean');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->Shows->insertData($data, "department");

            //user log submit
            $log_des='('. text_format($this->input->post('department_name'),30)  .')'. " Department created.";
            user_log($log_des,'admin/department');
	...
	public function edit($id,$count1)
    {
        ...
        $con = "id = '$id'";
		$data['count1'] = $count1;
        $data['array'] = $this->Shows->getThisValue($con, "department");       
	...
	function update()
    {
        ...
        if ($this->input->post('id')) {
            ...
            if (strtolower($this->input->post('department_name')) != strtolower($original_value[0]->department_name)) {
                $is_unique = '|is_unique[department.department_name]';
            } else {
                $is_unique = '';
            }
            // $this->form_validation->set_rules('wing_id', 'Wing ID', 'trim|required|xss_clean');
            $this->form_validation->set_rules('department_name', 'Department', 'trim|required|xss_clean'. $is_unique);
           if ($this->form_validation->run() == false) {
                $value = $this->form_validation->error_array();
                echo json_encode($value);
                ...
                $department_name = $this->input->post('department_name');                
                    ...
                    $con = "id = '$id'";
                    $this->Shows->updateThisValue($data, "department", $con);

                      //user log submit
                    $log_des='('. text_format($this->input->post('department_name'),30)  .')'. " Department updated.";
                    user_log($log_des,'admin/department');
	...
	function delete($id,$action)
    {
       ...
       //  $con = "id = '$id'";
       //  $result = $this->Shows->deleteThisValue($con,"department");       
       // echo json_encode(array("status" => "success", "message" => "Data Active Successfully!"));
        ...
        $result = $this->Shows->updateThisValue($data, "department", $con);
        //user log submit
        $original_value = $this->D_model->checkId($id);
        $log_des="(".$original_value[0]->department_name.")"." Department ".$status.".";
        user_log($log_des,'admin/department');
