...
class Section extends BaseController
{
    public function __construct()
    {
       ...
       $this->lang->load('config', $this->session->userdata('lang_file'));
        $this->load->model('admin/section_model', 'D_model');
    }
    ...
    public function index()
    {
        ...
        $config['base_url'] = base_url() . 'admin/section/index';
		...
		$this->data['dynamicView'] = "section/index";
   ...
   public function paginate_data()
    {
        ...
        $name = $this->input->post('section_search');      

        if (!empty($name)) {
            $conditions['search']['section_name'] = $name;
        }
        $totalRec = @count($this->D_model->get_user_data($conditions));

        //pagination configuration
        $config['target'] = '#sectionList';
        $config['base_url'] = base_url() . 'admin/section/paginate_data';
        $config['total_rows'] = $totalRec;
        $config['per_page'] = $this->total_row;
        $config['link_func'] = 'searchFilter';
        $this->ajax_pagination->initialize($config);
        ...
        $this->load->view('admin/section/home', $data);
	...
	public function add()
    {
        ...
        $this->load->view('admin/section/entry');
	...
	function added()
    {
        ...
        $this->form_validation->set_rules('section_name', 'Section', 'trim|required|is_unique[section.section_name]|xss_clean');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->Shows->insertData($data, "section");
              //user log submit
              $log_des='('. text_format($this->input->post('section_name'),30)  .')'. " section created.";
              user_log($log_des,'admin/section');
	...
	public function edit($id,$count1)
    {
        ...
        $con = "id = '$id'";
		$data['count1'] = $count1;
        $data['array'] = $this->Shows->getThisValue($con, "section");       
	...
	function update()
    {
        ...
        if ($this->input->post('id')) {
            ...
            $original_value = $this->D_model->checkId($id);
             if (strtolower($this->input->post('section_name')) != strtolower($original_value[0]->section_name)) {
                $is_unique = '|is_unique[section.section_name]';
            } else {
                $is_unique = '';
            }
            $this->form_validation->set_rules('section_name', 'Section', 'trim|required|xss_clean'. $is_unique);
           if ($this->form_validation->run() == false) {
                $value = $this->form_validation->error_array();
                echo json_encode($value);
                ...
                $designation_name = $this->input->post('section_name');                
                    $data = array(
                       'section_name' => $this->input->post('section_name'), 
                    );
                    $con = "id = '$id'";
                    $this->Shows->updateThisValue($data, "section", $con);

                    //user log submit
                    $log_des='('. text_format($this->input->post('section_name'),30)  .')'. " Section updated.";
                    user_log($log_des,'admin/section');
	...
	function delete($id,$action)
    {
        ...
        $result = $this->Shows->updateThisValue($data, "section", $con);
        //user log submit
        $original_value = $this->D_model->checkId($id);
        $log_des="(".$original_value[0]->designation_name.")"." section ".$status.".";
        user_log($log_des,'admin/section');
