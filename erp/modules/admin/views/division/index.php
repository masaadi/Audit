...
<div class="content-wrapper">
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="card pt-2" style="box-shadow: 0px 0px 3px #777;">
                <div class="card-body " >
                    <h4 class="text-center"><?php echo $this->lang->line('division'); ?></h4>
                    <div class="row p-3">
              ...
              data-keyboard="false"><img src="<?php echo base_url();?>public/img/plus.png"/><?php echo $this->lang->line('add_new'); ?></button>
                ...
                <div class="card search_form">
				  ...
				  <?php echo form_open_multipart('', array('id' => 'division_search')); ?>
                     
                    <div class="row " >
                        <div class="col-md-3">
                            <div class="form-group">
                              <label><?php echo $this->lang->line('division'); ?></label>
                              <input type="text" name="div_name" id="div_name" placeholder="<?php echo $this->lang->line('division_name'); ?>" class="form-control">
                            </div>
                          </div>
				 ...
				 <div  id="divisionList">
                    <?php
                    $this->load->view('admin/division/home');
                    ?>
                </div>
                <!-- table -->
       ...
       function searchFilter(page_num) {
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo base_url(); ?>admin/division/paginate_data/' + page_num,
	 ...
	 function add_master() {
      $.ajax({
        type: "POST",
        url: '<?php echo base_url() ?>admin/division/add/',
        beforeSend: function () {
          $('.crud_load').show()
      },
      complete: function () {
  ...
  function edit_master(id,page_no) {
      ...
      $.ajax({
		...
		url: '<?php echo base_url() ?>admin/division/edit/' + id+'/'+page_no,
        beforeSend: function () {
          $('.crud_load').show()
      },
      complete: function () {
  ...
  function delete_master(id,action) {
      ...
      $.ajax({
        type: "POST",
        url: '<?php echo base_url() ?>admin/division/delete/' + id + '/' +action,
        beforeSend: function () {
          $('.crud_load').show()
      },
      complete: function () {
...
<script type="text/javascript">
   ...
   submitHandler: function (form) {
    $("label.error").remove();
          ...
          var frm = $('#division_add');
                    // e.preventDefault();
                    var form = $(this);
                    //debugger;

                    $.ajax({
                      url: "<?php echo base_url() ?>admin/division/added",
                      type: 'POST',
                      data: frm.serialize(),
                        //dataType: "JSON",
