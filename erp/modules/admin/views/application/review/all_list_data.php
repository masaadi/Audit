<table id="examplee" class="table table-striped table-bordered table-responsive" style="width:100%">
  <thead>
      <tr>
          ...
          <th style="width: 240px;"><?= $this->lang->line('submission_date')?></th>
  ...
  <tbody>
    ...
    if(!empty($posts)){
      foreach ($posts as $value) {
        ...
        echo '<td>'.$org_name.'</td>';
        echo '<td>'.niceDate($value['created_date']).'</td>';
        echo '<td>';
        ...
        if($value['status']==1||$value['status']==4||$value['status']==5){
          echo '<button type="button" class="btn btn-sm btn-warning" onclick="show_id('. $value['id'].')" data-toggle="modal" data-target="#openModal">'.$this->lang->line('unpresanted').'</button>';
        }else if($value['status']==6){
          echo '<button type="button" class="btn btn-sm btn-success">'.$this->lang->line('presanted').'</button>';
        }
