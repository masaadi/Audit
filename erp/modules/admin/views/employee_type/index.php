...
<div class="content-wrapper">
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="card pt-2" style="box-shadow: 0px 0px 3px #777;">
                <div class="card-body " >
                    <h4 class="text-center">Employee Type</h4>
                    <div class="row p-3">
              ...
              data-keyboard="false"><img src="<?php echo base_url();?>public/img/plus.png"/><?= $this->lang->line('add_new')?></button>
                ...
                <div class="card search_form">
                    ...
                    <div class="row " >
                        <div class="col-md-3">
                            <div class="form-group">
                              ...
                              <input type="text" name="employee_type" id="employee_type" placeholder="Employee Type" class="form-control">
                            </div>
                          </div>
				 ...
				 <div  id="designationList">
                    <?php
                    $this->load->view('admin/employee_type/home');
                    ?>
                </div>
                <!-- table -->
       ...
       function searchFilter(page_num) {
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo base_url(); ?>admin/employee_type/paginate_data/' + page_num,
            data: 'page=' + page_num + '&employee_type=' + employee_type,
            /* beforeSend: function () {
                $('.crud_load').show()
            },
            complete: function () {
	 ...
	 function add_master() {
      $.ajax({
        type: "POST",
        url: '<?php echo base_url() ?>admin/employee_type/add/',
        beforeSend: function () {
          $('.crud_load').show()
      },
      complete: function () {
  ...
  function edit_master(id,page_no) {
      ...
      $.ajax({
		...
		url: '<?php echo base_url() ?>admin/employee_type/edit/' + id+'/'+page_no,
        beforeSend: function () {
          $('.crud_load').show()
      },
      complete: function () {
  ...
  function delete_master(id,action) {
      ...
      $.ajax({
        type: "POST",
        url: '<?php echo base_url() ?>admin/employee_type/delete/' + id + '/' +action,
        beforeSend: function () {
          $('.crud_load').show()
      },
      complete: function () {
...
<script type="text/javascript">
   ...
   submitHandler: function (form) {
    $("label.error").remove();
          ...
          var frm = $('#type_name_add');
                    // e.preventDefault();
                    var form = $(this);
                    //debugger;

                    $.ajax({
                      url: "<?php echo base_url() ?>admin/employee_type/added",
                      type: 'POST',
                      data: frm.serialize(),
                        //dataType: "JSON",
  ...
  submitHandler: function (form) {
    $("label.error").remove();
     ...
     var frm = $('#designation_edit');
                    ...
                    $.ajax({
                      url: "<?php echo base_url() ?>admin/employee_type/update/",
                      type: 'POST',
                      data: frm.serialize(),
                        //dataType: "JSON",
                        success: function (response) {
