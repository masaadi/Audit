...
<div class="content-wrapper">
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="card pt-2" style="box-shadow: 0px 0px 3px #777;">
                <div class="card-body " >
                    <h4 class="text-center"><?php echo $this->lang->line('blog'); ?> <?php echo $this->lang->line('topic'); ?></h4>
                    <div class="row p-3">
              ...
              data-keyboard="false"><img src="<?php echo base_url();?>public/img/plus.png"/><?php echo $this->lang->line('add_new'); ?></button>
                ...
                <div class="card search_form">
                    ...
                    <div class="row " >
                        <div class="col-md-3">
                            <div class="form-group">
                              <label><?php echo $this->lang->line('topic'); ?></label>
                              <input type="text" name="topic_name" id="topic_name" placeholder="<?php echo $this->lang->line('topic'); ?>" class="form-control">
                            </div>
                          </div>
				 ...
				 <div  id="blogtopicList">
                    <?php
                    $this->load->view('admin/blog_topic/home');
                    ?>
                </div>
                <!-- table -->
       ...
       function searchFilter(page_num) {
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo base_url(); ?>admin/blog_topic/paginate_data/' + page_num,
            data: 'page=' + page_num + '&topic_name=' + topic_name,
            /* beforeSend: function () {
                $('.crud_load').show()
            },
            complete: function () {
	 ...
	 function add_master() {
      $.ajax({
        type: "POST",
        url: '<?php echo base_url() ?>admin/blog_topic/add/',
        beforeSend: function () {
          $('.crud_load').show()
      },
      complete: function () {
  ...
  function edit_master(id,page_no) {
      ...
      $.ajax({
		...
		url: '<?php echo base_url() ?>admin/blog_topic/edit/' + id+'/'+page_no,
        beforeSend: function () {
          $('.crud_load').show()
      },
      complete: function () {
  ...
  function delete_master(id) {
      ...
      $.ajax({
        type: "POST",
        url: '<?php echo base_url() ?>admin/blog_topic/delete/' + id,
        beforeSend: function () {
          $('.crud_load').show()
      },
      complete: function () {
...
<script type="text/javascript">
      ...
      topic_name_bn: {
          required: 'Topic Name (Bangla) is required',
      },
   ...
   submitHandler: function (form) {
    $("label.error").remove();
          ...
          var frm = $('#topic_add');
                    ...
                    $.ajax({
                      url: "<?php echo base_url() ?>admin/blog_topic/added",
                      type: 'POST',
                      data: frm.serialize(),
                        //dataType: "JSON",
  ...
  submitHandler: function (form) {
    $("label.error").remove();
     ...
     var frm = $('#topic_edit');
                    ...
                    $.ajax({
                      url: "<?php echo base_url() ?>admin/blog_topic/update/",
                      type: 'POST',
                      data: frm.serialize(),
                        //dataType: "JSON",
                        success: function (response) {
