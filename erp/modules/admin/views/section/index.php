...
<div class="content-wrapper">
            ...
            <div class="col-md-12">
              ...
              data-keyboard="false"><img src="<?php echo base_url();?>public/img/plus.png"/><?= $this->lang->line('add_new')?></button>
                ...
                <div class="card search_form">
				  ...
				  <?php echo form_open_multipart('', array('id' => 'section_search')); ?>
                     
                    <div class="row " >
                        <div class="col-md-3">
                            <div class="form-group">
                              <label>Section</label>
                              <input type="text" name="section_search" id="section" placeholder="section name" class="form-control">
                            </div>
                          </div>
				 ...
				 <div  id="sectionList">
                    <?php
                    $this->load->view('admin/section/home');
                    ?>
                </div>
                <!-- table -->
       ...
       function searchFilter(page_num) {
        page_num = page_num ? page_num : 0;
         var section_search = $('#section').val();	
		
        // alert(off_name);
        $.ajax({
            type: 'POST',
            url: '<?php echo base_url(); ?>admin/section/paginate_data/' + page_num,
            data: 'page=' + page_num + '&section_search=' + section_search,
            success: function (html) {
				    $("#section_search").trigger("reset");
  ...
  function edit_master(id,page_no) {
      ...
      $.ajax({
        type: "POST",
		    url: '<?php echo base_url() ?>admin/section/edit/' + id+'/'+page_no,
        beforeSend: function () {
          $('.crud_load').show()
      },
      complete: function () {
  ...
  function delete_master(id,action) {
      ...
      $.ajax({
        type: "POST",
        url: '<?php echo base_url() ?>admin/section/delete/' + id + '/' +action,
        beforeSend: function () {
          $('.crud_load').show()
      },
      complete: function () {
   ...
   submitHandler: function (form) {
    $("label.error").remove();
          ...
          var frm = $('#section_add');
                    ...
                    $.ajax({
                      url: "<?php echo base_url() ?>admin/section/added",
                      type: 'POST',
                      data: frm.serialize(),
                        //dataType: "JSON",
  ...
  submitHandler: function (form) {
    $("label.error").remove();
     ...
     var frm = $('#designation_edit');
                    ...
                    $.ajax({
                      url: "<?php echo base_url() ?>admin/section/update/",
                      type: 'POST',
                      data: frm.serialize(),
                        //dataType: "JSON",
                        success: function (response) {
