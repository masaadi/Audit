...
<div class="content-wrapper">
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="card pt-2" style="box-shadow: 0px 0px 3px #777;">
                <div class="card-body " >
                    <h4 class="text-center"><?= $this->lang->line('department')?></h4>
                    <div class="row p-3">
              ...
              data-keyboard="false"><img src="<?php echo base_url();?>public/img/plus.png"/><?= $this->lang->line('add_new')?></button>
                ...
                <div class="card search_form">
                  <div class="card-body">
				  <?php echo form_open_multipart('', array('id' => 'department_search')); ?>
                     
                    <div class="row " >
                        <div class="col-md-3">
                            <div class="form-group">
                              <label><?= $this->lang->line('department')?></label>
                              <input type="text" name="dept_name" id="dept_name" placeholder="" class="form-control">
                            </div>
                          </div>
				 ...
				 <div  id="departmentList">
                    <?php
                    $this->load->view('admin/department/home');
                    ?>
                </div>
                <!-- table -->
       ...
       function searchFilter(page_num) {
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo base_url(); ?>admin/department/paginate_data/' + page_num,
            data: 'page=' + page_num + '&dept_name=' + dept_name,
            /* beforeSend: function () {
                $('.crud_load').show()
            },
            complete: function () {
            ...
            success: function (html) {
				$("#department_search").trigger("reset");
	 ...
	 function add_master() {
      $.ajax({
        type: "POST",
        url: '<?php echo base_url() ?>admin/department/add/',
        beforeSend: function () {
          $('.crud_load').show()
      },
      complete: function () {
  ...
  function edit_master(id,page_no) {
      ...
      $.ajax({
		...
		url: '<?php echo base_url() ?>admin/department/edit/' + id+'/'+page_no,
        beforeSend: function () {
          $('.crud_load').show()
      },
      complete: function () {
  ...
  function delete_master(id,action) {
      ...
      $.ajax({
        type: "POST",
        url: '<?php echo base_url() ?>admin/department/delete/' + id + '/' +action,
        beforeSend: function () {
          $('.crud_load').show()
      },
      complete: function () {
...
<script type="text/javascript">
   ...
   submitHandler: function (form) {
    $("label.error").remove();
          ...
          var frm = $('#department_add');
                    // e.preventDefault();
                    var form = $(this);
                    //debugger;

                    $.ajax({
                      url: "<?php echo base_url() ?>admin/department/added",
                      type: 'POST',
                      data: frm.serialize(),
                        //dataType: "JSON",
  ...
  submitHandler: function (form) {
    $("label.error").remove();
     ...
     var frm = $('#department_edit');
                    // e.preventDefault();
                    var form = $(this);
                    //debugger;

                    $.ajax({
                      url: "<?php echo base_url() ?>admin/department/update/",
                      type: 'POST',
                      data: frm.serialize(),
                        //dataType: "JSON",
                        success: function (response) {
