    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card pt-2" style="box-shadow: 0px 0px 3px #777;">
                <div class="card-body">
                    <h4 class="text-center">Seniority Information</h4>
                    <div class="row p-3">
                    ...
                    <?php
                        ...
                        $employee_id = $user_info->employee_id;
                         $is_emp = $this->db->where('employee_id',$employee_id)->get('seniority_info')->row();
                         if(empty($is_emp)):
                    ?>
                    <h2 class="add_button">
                    ...
                    <?php
                        $this->load->view('applicant/seniority_info/home');
                    ?>
                    </div>
                    <!-- table -->
                </div>
...
function searchFilter(page_num) {
    ...
    $.ajax({
        type: 'POST',
        url: '<?php echo base_url(); ?>applicant/seniority_info/paginate_data/' + page_num,
...
function add_master() {
    $.ajax({
        type: "POST",
        url: '<?php echo base_url() ?>applicant/seniority_info/add/',
        beforeSend: function() {
            $('.crud_load').show()
        },
        complete: function() {
...
function edit_master(id, page_no) {
    ...
    $.ajax({
        type: "POST",
        url: '<?php echo base_url() ?>applicant/seniority_info/edit/' + id + '/' + page_no,
        beforeSend: function() {
            $('.crud_load').show()
        },
        complete: function() {
...
function delete_master(id) {
    ...
    if (a) {
        $.ajax({
            type: "POST",
            url: '<?php echo base_url() ?>applicant/seniority_info/delete/' + id,
            beforeSend: function() {
                $('.crud_load').show()
            },
            complete: function() {
    ...
    $('body').on('click', '#submit', function() {
        ...
        $('#seniority_info_add').validate({
            ...
            submitHandler: function(form) {
                $("label.error").remove();
                var frm = $('#seniority_info_add');
                var form = $(this);
                var currentForm = $('#seniority_info_add')[0];
                var formData = new FormData(currentForm);
                //debugger;
                $.ajax({
                    url: "<?php echo base_url() ?>applicant/seniority_info/added",
                    type: 'POST',
                    processData: false,
                    contentType: false,
                    data: formData,
    ...
    $('body').on('click', '#edit_submit', function() {
        $('#personal_info_edit').validate({
            ...
            submitHandler: function(form) {
                ...
                $.ajax({
                    url: "<?php echo base_url() ?>applicant/seniority_info/update/",
                    type: 'POST',
                    processData: false,
                    contentType: false,
                    data: formData,
...
<script type="text/javascript">
    function show_master(id,page_no) {
      ...
      $.ajax({
        type: "POST",
        url: '<?php echo base_url() ?>applicant/seniority_info/show/' + id+'/'+page_no,
        beforeSend: function () {
          $('.crud_load').show()
      },
      complete: function () {
