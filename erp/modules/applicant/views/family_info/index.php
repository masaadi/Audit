                ...
                <div class="card-body">
                    ...
                    <?php
                        ...
                        $employee_id = $user_info->employee_id;
                         $is_emp = $this->db->where('employee_id',$employee_id)->get('family_info')->row();
                         if(empty($is_emp)):
                    ?>
                    <h2 class="add_button">
                    ...
                    <?php
                        $this->load->view('applicant/family_info/home');
                    ?>
                    </div>
                    <!-- table -->
                </div>
...
function searchFilter(page_num) {
    ...
    $.ajax({
        type: 'POST',
        url: '<?php echo base_url(); ?>applicant/family_info/paginate_data/' + page_num,
...
function add_master() {
    $.ajax({
        type: "POST",
        url: '<?php echo base_url() ?>applicant/family_info/add/',
        beforeSend: function() {
            $('.crud_load').show()
        },
        complete: function() {
...
function edit_master(id, page_no) {
    ...
    $.ajax({
        type: "POST",
        url: '<?php echo base_url() ?>applicant/family_info/edit/' + id + '/' + page_no,
        beforeSend: function() {
            $('.crud_load').show()
        },
        complete: function() {
    ...
    $('body').on('click', '#submit', function() {
        ...
        $('#family_info_add').validate({
            ...
            submitHandler: function(form) {
                $("label.error").remove();
                var frm = $('#family_info_add');
                var form = $(this);
                var currentForm = $('#family_info_add')[0];
                var formData = new FormData(currentForm);
                //debugger;

                $.ajax({
                    url: "<?php echo base_url() ?>applicant/family_info/added",
                    type: 'POST',
                    processData: false,
                    contentType: false,
                    data: formData,
    ...
    $('body').on('click', '#edit_submit', function() {
        $('#family_info_edit').validate({
            ...
            submitHandler: function(form) {
                ...
                $("label.error").remove();
                var frm = $('#family_info_edit');
                var form = $(this);
                var currentForm = $('#family_info_edit')[0];
                var formData = new FormData(currentForm);
                //debugger;

                $.ajax({
                    url: "<?php echo base_url() ?>applicant/family_info/update/",
                    type: 'POST',
                    processData: false,
                    contentType: false,
                    data: formData,
...
<script type="text/javascript">
    function show_master(id,page_no) {
      ...
      $.ajax({
        type: "POST",
        url: '<?php echo base_url() ?>applicant/family_info/show/' + id+'/'+page_no,
        beforeSend: function () {
          $('.crud_load').show()
      },
      complete: function () {
