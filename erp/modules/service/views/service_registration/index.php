        ...
        <div class="col-md-12">
            <div class="card pt-2" style="box-shadow: 0px 0px 3px #777;">
                <div class="card-body">
                    ...
                    <?php
                        $this->load->view('service/service_registration/home');
                    ?>
                    </div>
                    <!-- table -->
                </div>
...
function searchFilter(page_num) {
    ...
    $.ajax({
        type: 'POST',
        url: '<?php echo base_url(); ?>service/service_registration/paginate_data/' + page_num,
...
function add_master() {
    $.ajax({
        type: "POST",
        url: '<?php echo base_url() ?>service/service_registration/add/',
        beforeSend: function() {
            $('.crud_load').show()
        },
        complete: function() {
...
function edit_master(id, page_no) {
    ...
    $.ajax({
        type: "POST",
        url: '<?php echo base_url() ?>service/service_registration/edit/' + id + '/' + page_no,
        beforeSend: function() {
            $('.crud_load').show()
        },
        complete: function() {
...
function delete_master(id) {
    ...
    if (a) {
        $.ajax({
            type: "POST",
            url: '<?php echo base_url() ?>service/service_registration/delete/' + id,
            beforeSend: function() {
                $('.crud_load').show()
            },
            complete: function() {
    ...
    $('body').on('click', '#submit', function() {
        $('#personal_info_add').validate({
            ...
            submitHandler: function(form) {
                ...
                $.ajax({
                    url: "<?php echo base_url() ?>service/service_registration/added",
                    type: 'POST',
                    processData: false,
                    contentType: false,
                    data: formData,
    ...
    $('body').on('click', '#edit_submit', function() {
        $('#personal_info_edit').validate({
            ...
            submitHandler: function(form) {
                ...
                $.ajax({
                    url: "<?php echo base_url() ?>service/service_registration/update/",
                    type: 'POST',
                    processData: false,
                    contentType: false,
                    data: formData,
...
<script type="text/javascript">
  ...
  function show_query(id) {
      $.ajax({
        type: "POST",
        url: '<?php echo base_url() ?>service/service_registration/show_query/' + id,
      success: function (result) {
          if (result) {
              $('.search_form').hide();
             $('.add_button').hide();
  ...
  function show_status(id) {
      ...
      $.ajax({
        type: "POST",
        url: '<?php echo base_url() ?>service/service_registration/show_status/' + id,
      success: function (result) {
          if (result) {
              $('.search_form').hide();
             $('.add_button').hide();
