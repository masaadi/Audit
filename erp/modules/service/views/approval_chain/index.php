    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card pt-2" style="box-shadow: 0px 0px 3px #777;">
                <div class="card-body">
                    <h4 class="text-center">Approval Chain</h4>
                    <div class="row p-3">
                    ...
                    <?php
                        $this->load->view('service/approval_chain/home');
                    ?>
                    </div>
                    <!-- table -->
                </div>
...
function searchFilter(page_num) {
    ...
    $.ajax({
        type: 'POST',
        url: '<?php echo base_url(); ?>service/approval_chain/paginate_data/' + page_num,
...
function add_master() {
    $.ajax({
        type: "POST",
        url: '<?php echo base_url() ?>service/approval_chain/add/',
        beforeSend: function() {
            $('.crud_load').show()
        },
        complete: function() {
...
function edit_master(id, page_no) {
    ...
    $.ajax({
        type: "POST",
        url: '<?php echo base_url() ?>service/approval_chain/edit/' + id + '/' + page_no,
        beforeSend: function() {
            $('.crud_load').show()
        },
        complete: function() {
...
function delete_master(id,status) {
    ...
    if (a) {
        $.ajax({
            type: "POST",
            url: '<?php echo base_url() ?>service/approval_chain/delete/' + id+'/'+status,
            beforeSend: function() {
                $('.crud_load').show()
            },
            complete: function() {
    ...
    $('body').on('click', '#submit', function() {
        ...
        $('#personal_info_add').validate({
            ...
            submitHandler: function(form) {
                ...
                //debugger;
                $.ajax({
                    url: "<?php echo base_url() ?>service/approval_chain/added",
                    type: 'POST',
                    processData: false,
                    contentType: false,
                    data: formData,
    ...
    $('body').on('click', '#edit_submit', function() {
        $('#personal_info_edit').validate({
            ...
            submitHandler: function(form) {
                ...
                $.ajax({
                    url: "<?php echo base_url() ?>service/approval_chain/update/",
                    type: 'POST',
                    processData: false,
                    contentType: false,
                    data: formData,
...
<script type="text/javascript">
    function show_master(service_type_id,page_no) {
      ...
      $.ajax({
        type: "POST",
        url: '<?php echo base_url() ?>service/approval_chain/show/' + service_type_id+'/'+page_no,
        beforeSend: function () {
          $('.crud_load').show()
      },
      complete: function () {
