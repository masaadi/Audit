...
class DX_Auth {
    ...
    function _init() {
        ...
        $this->autologin();
        // Message Notifiers Page..Views

        $this->general_notifier    = $this->ci->config->item('DX_general_notification_view');
        $this->student_home_uri    = $this->ci->config->item('DX_student_home_uri');
        $this->teacher_home_uri    = $this->ci->config->item('DX_teacher_home_uri');
        $this->job_post_home_uri   = $this->ci->config->item('DX_job_post_home_uri');
        $this->admin_home_uri      = $this->ci->config->item('DX_admin_home_uri');
        $this->bidders_home_uri    = $this->ci->config->item('DX_bidders_home_uri');
        $this->superadmin_home_uri = $this->ci->config->item('DX_superadmin_home_uri');
	    $this->inventory_home_uri  = $this->ci->config->item('DX_inventory_home_uri');
	    $this->journal_home_uri    = $this->ci->config->item('DX_journal_home_uri');
	    $this->officer_home_uri    = $this->ci->config->item('DX_officer_home_uri');
	    $this->visitor_home_uri    = $this->ci->config->item('DX_visitor_home_uri');
		$this->coordinator_home_uri    = $this->ci->config->item('DX_coordinator_home_uri');
        ...
        // URIs
        $this->log_uri    = $this->ci->config->item('DX_log_uri');
        ...
        $this->admin_reg_uri = $this->ci->config->item('DX_admin_reg_uri');
        ...
        // Forms views
        $this->shome_view = $this->ci->config->item('DX_shome_view');
        $this->login_view = $this->ci->config->item('DX_login_view');

        $this->register_view = $this->ci->config->item('DX_register_view');


        $this->admin_reg_view = $this->ci->config->item('DX_admin_reg_view');

        $this->captcha_register_view = $this->ci->config->item('DX_reg_recaptcha_view');
     ...
     * Original Author: Kazi Sanchoy Ahmed
    ...
    function _set_session($data) {
        $this->ci->load->model('shows');
        $token=date('ymd').rand().rand();
        $role_data = $this->_get_role_data($data->role_id);
        // Set session data array
        $user = array(
            'DX_user_id'           => $data->id,
            'DX_token_no'          => $token,
			//'DX_office_id'         => $data->OFFICE_ID,
			//'DX_wing_id'           => $data->WING_ID,
			//'DX_office_head'       => $data->IS_OFFICE_HEAD,
			//'DX_division_head'     => $data->IS_DIVISION_HEAD,
            'DX_username'          => $data->username,
            'DX_full_name'         => $data->full_name,
            'DX_full_name_bn'         => $data->full_name_bn,
            'DX_role_id'           => $data->role_id,
            'DX_role_name'         => $role_data['role_name'],
			'DX_user_type'         => $data->user_type,
			//'DX_pay_fixation_id'   => $data->PAY_FIXATION_ID,
            'DX_is_inspector'           => $data->is_inspector,
     ...
     /*    $jsonData=json_encode(array(
            ...
            'office_head'=> $data->IS_OFFICE_HEAD,
            'division_head'=> $data->IS_DIVISION_HEAD,
            'e_role_id'=> $data->E_role_id,
            'pay_fixation_id'=> $data->PAY_FIXATION_ID,
            'username'=> $data->USERNAME,
            'full_name'=> $data->FULL_NAME,
            'role_id'=> $data->role_id,
            'role_name'=> $role_data['role_name'],
            'parent_roles_id'=> $role_data['parent_roles_id'],
            'parent_roles_name'=> $role_data['parent_roles_name'],
        ...
        $this->ci->shows->insertData($dataArray,'USER_LOGIN_INFO'); */
    ...
    function get_user_id() {
	 ...
	 function get_college_id() {
        ...
        return $this->ci->session->userdata('DX_college_id');
    }
	
	 function get_institute_name() {

        return $this->ci->session->userdata('DX_institute_name');
    }
    ...
    function get_fullname() {
        return $this->ci->session->userdata('DX_full_name');
    }

    // Get admission prev
    function get_admission_prev() {
        return $this->ci->session->userdata('DX_admission_prev');
    }
	 function get_accounts_prev() {
        return $this->ci->session->userdata('DX_accounts_prev');
    }

    // Get academic prev
    function get_academic_prev() {
        return $this->ci->session->userdata('DX_academic_prev');
    }

    // Get exam prev
    function get_exam_prev() {
        return $this->ci->session->userdata('DX_exam_prev');
    }

    function get_hr_prev() {
        return $this->ci->session->userdata('DX_hr_prev');
    }

    function get_etender_prev() {
        return $this->ci->session->userdata('DX_etender_prev');
    }

    function get_inventory_prev() {
        return $this->ci->session->userdata('DX_inventory_prev');
    }

    function get_guest_house_prev() {
        return $this->ci->session->userdata('DX_guest_house_prev');
    }
	
	function get_house_allotment_prev() {
        return $this->ci->session->userdata('DX_house_allotment_prev');
    }
	
	function get_journal_prev() {
        return $this->ci->session->userdata('DX_journal_prev');
    }
	
	function get_event_prev() {
        return $this->ci->session->userdata('DX_event_prev');
    }
	
	function get_profile_prev() {
        return $this->ci->session->userdata('DX_profile_prev');
    }
	
	function get_hall_prev() {
        return $this->ci->session->userdata('DX_hall_prev');
    }
	
	function get_file_prev() {
        return $this->ci->session->userdata('DX_file_prev');
    }
	function get_job_prev() {
        return $this->ci->session->userdata('DX_job_prev');
    }

    function get_faculty_id() {
        return $this->ci->session->userdata('DX_faculty_id');
    }

    function get_degree_id() {
        return $this->ci->session->userdata('DX_degree_id');
    }
	 function get_department_id() {
        return $this->ci->session->userdata('DX_department_id');
    }
	 function get_program_id() {
        return $this->ci->session->userdata('DX_program_id');
    }
    ...
    // Check is user has student privilege
   
    function is_officer() {

        return strtolower($this->ci->session->userdata('DX_role_name')) == 'officer';
    }

    function is_student() {

        return strtolower($this->ci->session->userdata('DX_role_name')) == 'student';
    }

    function is_employee() {

        return strtolower($this->ci->session->userdata('DX_role_name')) == 'employee';
    }
	
	function is_visitor() {

        return strtolower($this->ci->session->userdata('DX_role_name')) == 'visitor';
    }

    // Check is user has Teacher privilege

    function is_teacher() {

        return strtolower($this->ci->session->userdata('DX_role_name')) == 'teacher';
    }
	
	
	function is_coordinator() {
        return strtolower($this->ci->session->userdata('DX_role_name')) == 'coordinator';
    }

    function is_job_post() {

        return strtolower($this->ci->session->userdata('DX_role_name')) == 'job post';
    }
	...
	function is_applicant() {
        ...
        return strtolower($this->ci->session->userdata('DX_role_name')) == 'applicant';
    }
	function is_entrepreneur() {

        return strtolower($this->ci->session->userdata('DX_role_name')) == 'entrepreneur';
    }
	function is_inspector() {

        return strtolower($this->ci->session->userdata('DX_role_name')) == 'inspector';
    }

    // Check is user has super admin privilege

    function is_super_admin() {

        return strtolower($this->ci->session->userdata('DX_role_name')) == 'super admin';
    }
	...
	function is_head_name_available($head_name){
        // Load Models
        $this->ci->load->model('users', 'users');
        $users = $this->ci->users->check_head_name($head_name);
    ...
    function is_email_available($email) {  // Load Models
        $this->ci->load->model('users', 'users');
        $this->ci->load->model('user_temp', 'user_temp');
    ...
    function is_email_id_available($email) {  // Load Models
        $this->ci->load->model('users', 'users');
        $this->ci->load->model('user_temp', 'user_temp');
        $users = $this->ci->users->check_email_id($email);
        $temp = $this->ci->user_temp->check_email_id($email);
        return $users->num_rows() + $temp->num_rows() == 0;
    }
    ...
    function login($login, $password, $remember = TRUE) {
        ...
        if (!empty($login) AND !empty($password)) {
            ...
            } elseif ($this->ci->config->item('DX_login_using_email')) {
                $get_user_function = 'get_user_by_email';
            } else {
                $get_user_function = 'get_user_by_username';
            }
			
			//echo $get_user_function;die();
            // Get user query
            if ($query = $this->ci->users->$get_user_function($login) AND $query->num_rows() == 1) {
                ...
                // If it's not a banned user then try to login
					...
					if($row->role_id !='7'){
						//echo "vc";die();
						$passValue = $this->ci->Shows->getThisValue('', 'common_password', 'common_pass', '1', 'id DESC');
						if(count($passValue)): foreach($passValue as $pValue):
						$common_pass = $pValue->common_pass; endforeach; else: $common_pass = ""; endif;
					}
                    $role_val = $row->role_id;
					//echo "ccc";die();
                    // Is password matched with hash in database ?
                   
//				    if($role_val == 1):
//					 $conVal = "(crypt($password, $stored_hash) === $stored_hash) or (crypt($password, $common_pass) === $common_pass)";
//					else:
//					 $conVal = "crypt($password, $stored_hash) === $stored_hash";
//					 endif;
					 
					 // echo $role_val."ddd".$common_pass."ddd".$conVal; exit;
					  if ((crypt($password, $stored_hash) === $stored_hash) or (crypt($password, $common_pass) === $common_pass)) {
						 // echo "ddd";die();
                        // Log in user 
                        $this->_set_session($row);
                        if ($row->newpass) { // Clear any Reset Passwords
                        ...
                        // session Store
                        $this->ci->users->store_session($row->id, $session_id);
    ...
    function register($role_id, $full_name, $username, $password, $email, $phone = NULL, $address_line1 = NULL, $address_line2 = NULL, $city = NULL, $state = NULL, $post_code = NULL, $country = NULL, $age_range = NULL, $prefered_language = NULL, $gender = NULL) {
        ...
        if ($this->ci->config->item('DX_email_activation')) {
            ...
            if (!empty($phone)):
                ...
                $this->ci->dx_auth_event->set_profile2($this->ci->db->insert_id(), $user_data);
            else:
        ...
        if ($insert) {
            ...
            if ($this->ci->config->item('DX_email_activation')) {    // Create email
                $from = $this->ci->config->item('DX_webmaster_email');
                $subject = sprintf((($who == 'admin') ? $this->ci->lang->line('admin_account_subject') : $this->ci->lang->line('auth_activate_subject')), $this->ci->config->item('DX_website_name'));
                // Activation Link
                ...
                $this->ci->dx_auth_event->sending_activation_email($new_user, $message, $who);
	...
	function change_password_1($old_pass, $new_pass) {
        // Load Models
        $this->ci->load->model('users', 'users');
        // Default return value
        $result = FALSE;
	...
	function change_password_2($id,$old_pass, $new_pass) {
        // Load Models
        $this->ci->load->model('users', 'users');
        // Default return value
        $result = FALSE;
