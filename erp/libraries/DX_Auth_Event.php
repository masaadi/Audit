 ...
 * 		@File Type		DX Auth Event Public Class															*
...

{
	...
	function set_profile2($user_id, $data)
	{
	// Load models
		$this->ci->load->model('user_profile', 'user_profile');
		// Create user profile
		$this->ci->user_profile->set_profile($user_id, $data);
	}
...

	{
		...
		$this->ci->users->login_status($user_id, $data);
	}
	...
	function sending_activation_email($data, &$content, $who)

	{
		...
		if($who!=='admin'){
			...
			$content = sprintf($this->ci->lang->line('admin_account_content'), 
