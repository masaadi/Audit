...
class Users extends CI_Model 
{
	...
	// session Store
    function store_session($user_id, $session_id) {
        $data = array(
	...
	function login_status($user_id, $data)
	{
		$this->db->where('id', $user_id);
        $this->db->update($this->_table, $data);
	}
    ...
    {
        return $this->db->query("select * from users where mobile = " . $mobile)->result();
    }
	public function get_user_by_otp($otp)
    {
        return $this->db->query("select * from users where otp = " . $otp)->result();
    }
}